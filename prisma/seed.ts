...
const seedGameTypes = async (userId: string) => {
  ...
  existingNlheSeed
    ? await prisma.gameType
        ...
        .catch(handleError)
        .then((gameType) =>
          console.log(`game type ${gameType.name} updated`, gameType)
        )
    : await prisma.gameType
        .create({
          data: nlheSeedData,
        ...
        .catch(handleError)
        .then((gameType) =>
          console.log(`game type ${gameType.name} created`, gameType)
        );
...
const seedGameFormats = async (userId: string) => {
  ...
  existingFreezeoutSeed
    ? await prisma.gameFormat
        ...
        .catch(handleError)
        .then((gameFormat) =>
          console.log(`game format ${gameFormat.name} updated`, gameFormat)
        )
    : await prisma.gameFormat
        .create({
          data: freezeoutSeedData,
        ...
        .catch(handleError)
        .then((gameFormat) =>
          console.log(`game format ${gameFormat.name} created`, gameFormat)
        );
