...
interface BusyFormInputProps {
    isBusy: boolean | undefined;
    busyMessage: string;
}

type FormInputProps = CommonFormInputProps & (BusyFormInputProps | Never<BusyFormInputProps>);

const FormInput = forwardRef<FormInputHandle, FormInputProps>(
    ({ busyMessage, isBusy, label, options }, forwardedRef) => {
        ...
        const styles = {
            busy: 'animate-pulse text-sm',
            icon: 'h-5 w-5',
            iconContainer: 'absolute top-2.5 right-2 flex space-x-2',
            ...
            <div>
                ...
                <div className={styles.inputContainer}>
                    ...
                    <div className={styles.iconContainer}>
                            ...
                            >
                                {isHidingPassword && (
                                    <EyeSlashIcon
                                        className={clsx(
                                            styles.icon,
                                            isDisabled ? styles.iconDisabled : styles.iconEnabled
                                        )}
                                    />
                                )}
                                {isShowingPassword && (
                        ...
                        {inputValue && (
                            <Button
                                options={{ isDisabled: isDisabled, onPress: onClear }}
                                size={'none'}
                                variant={'icon'}
                            >
                                <XMarkIcon
                ...
                {/* BUSY MESSAGE */}
                {isBusy && <p className={styles.busy}>{busyMessage}</p>}
